...
export default function GenerateTargets() {
  ...
  useEffect(() => {
    apiFetch('/challenges').then((data) => {
      setChallenges(data.map(c => ({ ...c })));
      return apiFetch(`/challenges/${data[0]?.slug}`);
    }).then(() => {
      return apiFetch('/challenges').then(async (summaries) => {
        const full = [];
        for (const s of summaries) {
          const c = await apiFetch(`/challenges/${s.slug}`);
          full.push(c);
        }
        setChallenges(full);
      });
  ...
  const generate = async (challenge) => {
    const iframe = document.createElement('iframe');
    iframe.style.position = 'fixed';
    iframe.style.top = '-9999px';
    iframe.width = challenge.viewport_w;
    iframe.height = challenge.viewport_h;
